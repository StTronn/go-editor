...
// Temp func till crdt is implemented
func (e *Editor) PerformOperation(opType int, ch rune) {
	switch opType {
	case OperationInsert:
		e.Text = InsertAt(e.Text, e.Cursor, ch)
		e.MoveCursor(1, 0)
	...
	case OperationDelete:
		...
		e.Text = DeleteAt(e.Text, e.Cursor)
		e.MoveCursor(-1, 0)
...
func InsertAt(slice []rune, index int, value rune) []rune {
	...
	// Append an empty slot to the slice
	slice = append(slice, 0)

	// Move the elements down to create a space for the new element
...
func DeleteAt(slice []rune, index int) []rune {
	// Check if the index is within the bounds of the slice
	if index < 0 {
		index = 0
	}
	...
	// Remove the element by reslicing
